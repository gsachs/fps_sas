...
- `docs/solutions/logic-errors/minimap-rotation-composition-sign-error.md` — a different bug (`src/ui/minimap.js`'s rotation-matrix composition, not a basis-vector choice) in a different subsystem, but this doc's own prevention lesson directly motivated that one's regression test: "cross-check against the actual rendered/consuming side, not just internal orthonormality" led to a directional test that caught a second, independent instance of the same higher-level pattern (an internally-valid coordinate transform, wrong relative to its real consumer, invisible to a magnitude/self-consistency check) before it ever shipped.
